...
if [[ $# -eq 0 ]]; then
  echo 'usage:'
  echo '    docker-compose -f production.yml run db restore <backup-file>'
  echo ''
  echo 'to get a list of available backups, run:'
  echo '    docker-compose -f production.yml run db list-backups'
  exit 1
fi
...
if dropdb -h db -U $POSTGRES_USER $POSTGRES_DB; then
  echo "deleted $POSTGRES_DB database"
else
  echo "database $POSTGRES_DB does not exist, continue"
fi

# create a new database
echo "creating new database $POSTGRES_DB"
createdb -h db -U $POSTGRES_USER $POSTGRES_DB -O $POSTGRES_USER

# restore the database
echo "restoring database $POSTGRES_USER"
gunzip -c $BACKUPFILE | psql -h db -U $POSTGRES_USER $POSTGRES_DB
